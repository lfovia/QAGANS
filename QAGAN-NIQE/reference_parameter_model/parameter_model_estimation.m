%% threshold to select the patches %%%%%%%%%%
sh_th = 0.75;
blocksizerow = 32;
blocksizecol = 32;
blocksizedepth = 8;
blockrowoverlap = 0;
blockcoloverlap = 0;
%%%%%%%%%% the path of real images 
path='/home/parimala/Desktop/Neurips_Reproducibility_NIQE_CIfar/cifar_model_parameters/train/';
F1 = dir(path);
nameFolds = {F1.name}';
nameFolds(ismember(nameFolds,{'.','..'})) = [];
number_files=size(nameFolds,1);
feat_GAN=[];
featnum      = 18;
% ---------------------------------------------------------------
% Make the directory for storing the features
mkdir(sprintf('local_risquee_prisfeatures_cifar_all'))
no_images = 50000
for itr = 1:1:no_images
%itr
filename=char(nameFolds(itr));
    filename2=strcat(path,filename);
    im=imread(filename2);
    
if(size(im,3)==3)
im               = rgb2gray(im);
end
im = imgradient(im,'sobel');
im               = double(im);             
[row col]        = size(im);
block_rownum     = floor(row/blocksizerow);
block_colnum     = floor(col/blocksizecol);
im               = im(1:block_rownum*blocksizerow, ...
                   1:block_colnum*blocksizecol);               
window           = fspecial('gaussian',7,7/6);
window           = window/sum(sum(window));
scalenum         = 2;
warning('off')

feat = [];


for itr_scale = 1:scalenum

    
mu                       = imfilter(im,window,'replicate');
mu_sq                    = mu.*mu;
sigma                    = sqrt(abs(imfilter(im.*im,window,'replicate') - mu_sq));
structdis                = (im-mu)./(sigma+1);
              
               
               
feat_scale               = blkproc(structdis,[blocksizerow/itr_scale blocksizecol/itr_scale], ...
                           [blockrowoverlap/itr_scale blockcoloverlap/itr_scale], ...
                           @computefeature);
feat_scale               = reshape(feat_scale,[featnum ....
                           size(feat_scale,1)*size(feat_scale,2)/featnum]);
feat_scale               = feat_scale';


if(itr_scale == 1)
sharpness                = blkproc(sigma,[blocksizerow blocksizecol], ...
                           [blockrowoverlap blockcoloverlap],@computemean);
sharpness                = sharpness(:);
end

sh_th = 0.75;
feat                     = [feat feat_scale];

im =imresize(im,0.5);

end
save(sprintf('./local_risquee_prisfeatures_cifar_all/imagefeatures_local%d.mat',...
    itr),'feat','sharpness');
end
prisparam    = [];

for itr      = 1:1:no_images

% Load the features and select the only features     
load(strcat('./local_risquee_prisfeatures_cifar_all/imagefeatures_local',num2str(itr),'.mat'));
IX               = find(sharpness(:) >sh_th*max(sharpness(:)));
feat             = feat(IX,:);
prisparam        = [prisparam; feat];

end 
mu_prisparam       = nanmean(prisparam);
cov_prisparam      = nancov(prisparam);
%----------------------------------------------
% Save features in the mat file
save('model_cifar_all.mat','mu_prisparam','cov_prisparam');
%----------------------------------------------
