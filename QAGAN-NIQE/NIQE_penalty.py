...
def niqe(img,res):
	...
	#
	#model_mat = scipy.io.loadmat('./features_mu_real.mat')
	#model_mu = model_mat['features_mu_real_grad']
	model_mat = scipy.io.loadmat('./reference_parameter_model/model_face_grad.mat')
	#model_mat = scipy.io.loadmat('features_cov_real.mat')
	model_mu = model_mat['mu_prisparam']
	#model_mat2 = scipy.io.loadmat('./reference_parameter_model/Model_parmeters_cov_real_grad.mat')
	...
	model_cov_2 = model_cov
	model_mu_2=tf.convert_to_tensor(model_mu,dtype=tf.float32) 
	pseudoinv_of_avg_cov  = tf.linalg.inv((features_cov + model_cov_2)/2)
	first_matrix  = tf.matmul((model_mu_2 - features_mu),pseudoinv_of_avg_cov)
	finalscore = tf.sqrt(tf.matmul(first_matrix,(tf.transpose(model_mu_2 - features_mu))))
	return finalscore
